...
- `market-data.js`：可选共享服务连接、个人密钥隔离、保留缓存真实获取时间。
- `styles.css`：浅色/深色卡片和两列数据、紧凑层级、图表右侧刻度、手机删除按钮。
- `index.html`、`manifest.webmanifest`、`sw.js`：统一 v7 版本；只清理本应用缓存；API 不走离线缓存；移除 SW 强制跳转，保留一次刷新保护。
...
前端可继续直接部署 GitHub Pages，已有个人 Key 继续可用。共享后台代码已经完成，但 **未上线后台服务**：当前没有常驻服务器及服务端行情密钥，GitHub Pages 本身不能运行 Node。部署后填入设置即可免手机 Key 读取共享缓存；不承诺免费实时行情，实际到账仍需确认。
